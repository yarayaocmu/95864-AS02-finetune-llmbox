...
Protocol: the author rated all 100 held-out prompts and Experiment~2's answers against the criterion and Likert rubric of Section~2.4, seeing only the drug, condition, patient report and the model's answer (no reference label, no judge verdict); the sheet was then converted to LLMBox's \code{labeled\_evaluation\_dataset.json} format and compared with the judge using LLMBox's \code{AgreementMeasures} (percent agreement and Cohen's kappa).

\textbf{Results.} Mean rating 4.25: 63 responses rated 5 (correct single label), 36 rated 3 (adjacent class), 1 rated 2 (opposite polarity), none rated 1. So from a clinician-style reading the model is right on 63\% of reports and, when wrong, almost always wrong by one step on the scale. Human and LLM judge agree on 82\% of ratings with kappa 0.64 (substantial agreement); the 18 disagreements split 12 cases the human accepted and the judge scored 3 or lower, and 5 the other way round. Human agreement with the rubric applied using the \emph{rating-derived} reference label is lower, 68\% with kappa 0.32, and the reference ``accuracy'' on the same 100 prompts is 0.62. In other words, two independent readers of the text (a person and a 3.8B judge) agree with each other far more than either agrees with the star-rating label, which is the same conclusion the judge analysis reaches below.

\begin{table}[H]\centering\small
\begin{tabular}{lccccc}\toprule
& 1 & 2 & 3 & 4 & 5 \\\midrule
human ratings ($n=100$) & 0 & 1 & 36 & 0 & 63 \\
...
No AI tools were used in the preparation of this project. All code, analysis, experiments, and writing were completed independently by the author.
